...
// The key here is that they use an array method, Though they can just do artists[8].name = "Vincent Van Gogh", that doesn't follow MVP instructions
artists.splice(8, 1, {
  id: 8,
...
// There are a few ways to solve this one. They can use split, or slice, in order to grab the birth and death years. Here's how I approached it:

function get20s(array) {
  //create an array to hold the artists that match the criteria
  let twentiesArtists = [];
  for (let i = 0; i < array.length; i++) {
    //grab the birth and death year values out of the years string
    let birthYear = array[i].years.slice(0, 4);
    let deathYear = array[i].years.slice(-4);
    // make sure the birth and death years fall within the range
    if (birthYear > 1899 && birthYear < 2001) {
      if (deathYear < 2001) {
...
// make sure they created the object outside of the addArtist function, and that addArtist takes in 2 arguments
let newArtist = {
  id: 20,
  name: 'Your Name Here',
  years: 'Your Birth Year - current day',
